...
public class EnemyBase : PhysicsObject {
	...
	[SerializeField] enum EnemyType {Basic, Scorpions};
    ...
    protected override void ComputeVelocity()
    {
		...
		playerDifference = player.gameObject.transform.position.x - transform.position.x;
		...
		if (!recovering) {
			...
			if (enemyType == EnemyType.Scorpions) {
	...
	*/
	...
	private void OnCollisionEnter2D(Collision2D other)
	{
		if (other.gameObject.CompareTag("Player"))
		{
			...
			launch = launchDirection*(launchPower/10);
			recoveryCounter = 1;
			recovering = true;
		}
	}
